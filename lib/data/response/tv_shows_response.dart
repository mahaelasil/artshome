...
class TvShowsResponse {
  ...
  TvShowsResponse.fromJson(dynamic json) {
    ...
    if (json['results'] != null) {
      ...
      json['results'].forEach((v) {
        _results?.add(StareResults.fromJson(v));
      });
    }
  ...
  num? _page;
  List<StareResults>? _results;
  num? _totalPages;
  num? _totalResults;

  num get page => _page ?? 0;
  List<StareResults> get results => _results ?? [];
  num get totalPages => _totalPages ?? 0;
  num get totalResults => _totalResults ?? 0;

}

StareResults resultsFromJson(String str) => StareResults.fromJson(json.decode(str));
class StareResults {
  StareResults({
    String? backdropPath,
    String? firstAirDate,
  ...
  StareResults.fromJson(dynamic json) {
